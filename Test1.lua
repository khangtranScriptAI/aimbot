...
-- MOVE GUI (CHỮ NHẬT – DRAG TỰ DO)
local moveGui = Instance.new("Frame")
moveGui.Size = UDim2.new(0,110,0,40)
moveGui.Position = UDim2.new(0,140,0,200)
moveGui.BackgroundColor3 = Color3.fromRGB(60,60,60)
moveGui.BorderSizePixel = 0
moveGui.Parent = gui

Instance.new("UICorner", moveGui).CornerRadius = UDim.new(0,8)
...
lockButton.MouseButton1Click:Connect(function()
 ...
 if locking then
  ...
  if myRoot then
   local offset =
    camera.CFrame.Position - myRoot.Position
...
RunService.RenderStepped:Connect(function()
 ...
 and target.Character:FindFirstChild("HumanoidRootPart") then
  ...
  local dir =
   (targetRoot.Position - myRoot.Position).Unit
  ...
  camera.CFrame =
   CFrame.new(camPos, targetRoot.Position)
 end
end)
```
